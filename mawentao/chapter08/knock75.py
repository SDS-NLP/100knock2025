...
def collate_fn(batch):
    """
    DataLoaderのcollate_fnとして使用される関数。
    ミニバッチ内の各事例をパディングし、長さ順に並び替える。

    Args:
        batch (list of dict): ミニバッチに含まれる事例のリスト。
                              各事例は {'input_ids': Tensor, 'label': Tensor} 形式。

    Returns:
        dict: 結合され、パディング・ソートされた input_ids と label を含む辞書。
    """
...
print("\n--- collate_fn の出力例 (最初のバッチ) ---")

temp_loader = DataLoader(train_dataset, batch_size=4, shuffle=False, collate_fn=collate_fn)
first_batch = next(iter(temp_loader))
print("input_ids:\n", first_batch['input_ids'])
print("label:\n", first_batch['label'])
print("形状 (input_ids):", first_batch['input_ids'].shape)
print("形状 (label):", first_batch['label'].shape)
